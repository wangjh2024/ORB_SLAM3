...
namespace ORB_SLAM3
{
	...
	string System::CalculateCheckSum(string filename, int type)
	{
		...
		if (!CryptAcquireContext(&hProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
			cout << "[E] Error acquiring crypto context" << endl;
			f.close();
			return checksum;
		}

		if (!CryptCreateHash(hProv, CALG_MD5, 0, 0, &hHash)) {
			cout << "[E] Error creating MD5 hash" << endl;
		...
		while (int count = f.readsome(buffer, sizeof(buffer))) {
			if (!CryptHashData(hHash, (BYTE*)buffer, count, 0)) {
				cout << "[E] Error hashing data" << endl;
		...
		bResult = CryptGetHashParam(hHash, HP_HASHVAL, c, &dwHashLen, 0);

		CryptDestroyHash(hHash);
		CryptReleaseContext(hProv, 0);

		if (!bResult) {
			cout << "[E] Error getting hash value" << endl;
			return checksum;
		}
#else
		MD5_CTX md5Context;
